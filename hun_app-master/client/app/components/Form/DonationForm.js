...
class DonationForm extends Component {
  ...
  handleSubmitForm(event) {
    ...
    const donation_amount = this.state.donation_amount
    const xhr = new XMLHttpRequest();
    xhr.open('post', apiRoutes.donate);
    ...
    xhr.addEventListener('load', () => {
      if (xhr.status === 200) {
        // alert(xhr.response.data.url)
        // Auth.authenticateUser(xhr.response.data.user, xhr.response.data.token.accessToken);
        // history.goBack();
        window.open(xhr.response.data.url);
      } else {
        alert(xhr.response)
      }
    });
    xhr.send(JSON.stringify({ name, email, phone, address, price: donation_amount }));
                  ...
                  <div className="container">
                    <div className="content">
                      <div className="donation-form-section">
                        ...
                        <form className="email-login" className="donation-form-pay" onSubmit={event => this.handleSubmitForm(event)}>
                          ...
                          <div className="u-form-group confirm-info-input">
                            ...
                            <input
                              placeholder="Số điện thoại (e.g. 08/20)"
                              type="text"
                              name="phone"
...

                          {
                            this.state.one_time_donation ? (
                              <div className="u-form-group">
                                ...
                                <button type="submit">Đóng góp một lẩn</button>
                              </div>
                            ) : (<div className="u-form-group">
